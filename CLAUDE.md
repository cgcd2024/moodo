...
- **관리자 인증**: `POST /api/admin/login`으로 비밀번호 검증 후 sessionStorage에 키 저장, axios 인터셉터가 `x-admin-key` 헤더 자동 첨부. 등록/수정/삭제 라우트는 `requireAdmin` 미들웨어로 보호. 키는 `backend/.env`의 `ADMIN_KEY` (미설정 시 기본값 — `backend/middleware/auth.js` 참고, 배포 후 변경 권장).

...
- **API 주소**: 반드시 `src/lib/api.js`의 `api`(axios 인스턴스)와 `imageUrl()`을 사용. 컴포넌트에 URL 하드코딩 금지. `VITE_API_URL` 환경변수로 교체 가능.
...
- 로고/파비콘은 무한도전 모티브의 오리지널 드로잉 — MBC 원본(로고, 무도리)을 그대로 복제하지 말 것. `src/components/Logo.jsx`와 `public/favicon.svg`는 항상 같은 도형을 유지.
...
- 짤 크롤링 수집 예정.
- HTTPS 전환 시 `VITE_API_URL`을 https 도메인으로 바꿔야 함 (mixed content).
